...
export class SidebarComponent {
  ...
  classes = {
    container: `transition-[width] h-full relative flex-shrink-0 w-[50px] sm:w-[70px]`,
  ...
  ngOnInit() {
    this.themeService.isDark.subscribe((isDark) => {
      ...
      if (meta) {
        if (isDark) {
          return meta.setAttribute("content", "#242424");
        } else return meta.setAttribute("content", "#fff");
      }
    });
  }
